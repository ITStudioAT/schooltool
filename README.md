...
| `gitsave "Beschreibung" ["Version"] -Full` | Windows, `main` | Optional zusätzlich die vollständigen lokalen Releaseprüfungen ausführen. Die GitHub-Freigabe für das Live-Deployment bleibt erforderlich. |
...
| `gitpreview resume <Bundle-ID>` | Derselbe Windows-PC und Benutzer, ursprünglicher Projektordner | Setzt einen vollständig geprüften, vor der Veröffentlichung abgebrochenen Kandidaten ohne erneuten Testlauf fort. Prüft Nachweis, Paket, Quellstand und aktuelle GitHub-Branches erneut; veröffentlicht erst nach `PREVIEW`. Optional `-RefreshData` mit zusätzlichem `REFRESH`. |
| `gitrelease "Beschreibung"` | Windows, gespeichertes Feature | Prüft einen separaten Merge-Kandidaten mit aktuellem `main`. Nach `RELEASE` auf GitHub veröffentlichen, das vollständig integrierte Feature schließen und lokal zu `main` wechseln. Bereitet danach lokale Abhängigkeiten und Frontend vor, ohne Migrationen oder Seeder. Kein Live-Deployment. |
...
| `gitdeploy` | Windows, sauberes Projekt | Verlangt die erfolgreichen Pflichtprüfungen für exakt den aktuellen GitHub-Release. Prüft nach `LIVE` erneut und installiert genau diesen Stand über SSH, einschließlich vorgesehener Live-Datenbankschritte. Bei fehlenden, laufenden oder gescheiterten Prüfungen Abbruch. |
...
| `composer pdeploy` | Cloudways, intern durch `gitdeploy` | Benötigt die vom geprüften SSH-Aufruf übergebenen Release-Identitäten und CI-Laufdaten. Ein bloßer manueller Aufruf ist gesperrt; im Alltag `gitdeploy` am PC verwenden. |
...
**Gespeichert ist noch nicht live freigegeben.** Der normale Ablauf auf `main` ist `gitsave` → erfolgreiche GitHub-Prüfungen → `gitdeploy`. Bei Anwendungscode führt GitHub PHP-Tests mit Coverage, Formatprüfung und statische Analyse, Frontend-Tests und Build sowie Linux-Infrastrukturtests und native Windows-Workflowtests aus. PHP-Dateien laufen in begrenzten, sequenziellen Batches; getrennte Jobs besitzen getrennte Ressourcen. Native lokale MySQL-Provisionierung durch den Windows-Testdatenbankhelfer bleibt eine gesonderte Integrationsprüfung und wird nicht durch die GitHub-Freigabe bescheinigt.

Nur eindeutig reine Dokumentations- und Versionsänderungen können den schnellen CI-Weg nutzen. Dafür braucht es einen mit der neuen Prüfregel vollständig geprüften Vorfahren, unveränderte Prüfregeln, ausschließlich erlaubte Dokumentationsdateien bzw. das exakte Versionsliteral in `config/schooltool.php` und gleiche Frontend-Inhalte außer dem Commit-Marker. PHP-Syntax/Format, gezielte Sicherheitstests, Encoding, Abhängigkeiten und Release-Integrität werden trotzdem geprüft. Jede unklare Änderung verlangt die vollständigen CI-Prüfungen. Eine neue Patchnummer allein erlaubt keine Ausnahme.

`gitdeploy` benötigt eine angemeldete GitHub CLI (`gh`) mit Lesezugriff auf die Actions der Schooltool-Repository. Es akzeptiert ausschließlich den passenden Workflow, den exakten Release-Commit und den letzten erfolgreichen Prüfversuch mit allen für diesen Stand erforderlichen Jobs. Alte READY-Meldungen mit übersprungenen Anwendungstests reichen nicht. Es gibt keinen Schalter zum Umgehen dieser Freigabe und keine automatische Übernahme ungebundener Entwicklungsprüfungen.

`gitpreview`, `gitrelease` und die ausdrücklich gewählte Option `-Full` behalten ihre lokalen PHP-/Frontendtests, statische Analyse und Buildprüfungen in eigenen Arbeitsverzeichnissen und selbst angelegten Testdatenbanken. Fachliche Online-Tests bleiben erforderlich. Die CI-Laufdaten im Serveraufruf sind Übergabemetadaten eines zuvor geprüften SSH-Aufrufs, keine serverseitig verifizierte Signatur. Eine bereits laufende ältere Serverversion erhält den neuen Schutz gegen einen ungeprüften manuellen `pdeploy` erst mit ihrem nächsten bewusst ausgeführten Deployment.

Nach `prepare` oder einer leeren/falschen Bestätigung zeigt `gitpreview` den vollständigen Fortsetzungsbefehl an. Der erfolgreiche Prüfnachweis wird als Windows-DPAPI-geschütztes JSON in `.git/schooltool-preview/<Bundle-ID>.receipt` gespeichert und bindet Original-Checkout, Feature-Reservierung, Main, Quellstand, Kandidat und Paket-Prüfsumme. Kandidat und Paket aufbewahren. Änderungen an diesen Ständen, ungesicherte Arbeit oder ein fehlender/ungültiger Nachweis stoppen die Wiederaufnahme; es werden nicht automatisch Tests neu gestartet. Ein neuer Quellstand benötigt wieder vollständige Prüfungen. Nach Beginn eines Veröffentlichungsversuchs sperrt eine lokale `.started`-Markierung die automatische Wiederholung; Git- und Serverzustand müssen dann gezielt geprüft werden.
...
Bereits separat geprüfte ältere Nachweise (`legacy-reviewed` und die fest begrenzten `reviewed-patch`-Umfänge) behalten ihre Herkunfts-, Hash- und Quelldelta-Prüfungen. Lose Textlogs werden nicht automatisch übernommen. Neue Nachweise setzen immer die vollständigen aktuellen Prüfungen voraus; es gibt keinen Schalter zum Überspringen von Tests.
...
4. Bei bereits eingerichtetem, gepinntem Cloudways-Launcher nach erfolgreicher neuer GitHub-Freigabe `gitdeploy` verwenden. Ein noch nicht eingerichteter Server benötigt eine separat geprüfte Erstinstallation; der ungeprüfte manuelle `composer pdeploy` ist kein Ersatz für die Freigabe. Die aktuelle laufende Anwendung wird durch das Aktualisieren der PC-Helfer allein nicht verändert.
...
| Hauptanwendung korrigieren | Feature sichern → `gitmain` → korrigieren → `gitsave "Fehler behoben"` → GitHub-Prüfungen erfolgreich → `gitdeploy` |
...
| Feature freigeben | `gitsave "Funktion fertig"` → `gitrelease "Neue Funktion"` → GitHub-Prüfungen erfolgreich → `gitdeploy` |
