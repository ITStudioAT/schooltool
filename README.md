...
| `gitpreview resume <Bundle-ID> -Feature "neue-funktion"` | Derselbe Windows-PC und Benutzer, ursprünglicher Projektordner | Setzt einen vollständig geprüften, vor der Veröffentlichung abgebrochenen Kandidaten ohne erneuten Testlauf fort. Prüft Nachweis, Paket, Quellstand und aktuelle GitHub-Branches erneut; veröffentlicht erst nach `PREVIEW`. Datenersatz verlangt zusätzlich `REFRESH`. |
