...
## Use one reserved feature and publish only checked exact Git states
main and at most one feature/* are the user branches; codex/active-feature is an atomic reservation containing an immutable lifecycle id. gitsave commits/pushes only the current branch; main requires full PHP/UI/static/build checks and versioned saves require release notes/docs. gitpreview and gitrelease prepare preserved candidate worktrees, merge current main there and run mandatory full checks before explicit PREVIEW/RELEASE confirmation. Release atomically advances main and removes only the exact checked feature/reservation, with ancestry checks and compare-and-swap leases; never rewrite history. Branch switches prepare dependencies/build/cache only, without migrations or seeds. Candidate tests use only self-created disposable local databases, never the existing developer/test database. gitdeploy requires LIVE confirmation and key-only strict-host-verified SSH, pinning both frontend and backend source identity; pdeploy remains a manual main-only fallback. Preview import/deployment failure stays closed for explicit recovery; no automatic Git/database rollback is assumed.
