            ...
            <v-list-item v-for="(work, index) in teaching_works" :key="index" class="px-0">
                <div class="d-flex flex-column w-100">
                    ...
                    <div class="d-flex align-center ga-2 mt-1">
                        ...
                        <v-chip v-if="work.default_grade" size="x-small" color="success" variant="tonal">
                            Standardnote: {{ work.default_grade }}
                        </v-chip>
                    </div>
        ...
        <v-card tile flat color="transparent" class="w-100">
            <v-card-text>
                <v-form ref="form" v-model="is_valid" @submit.prevent="save" class="mb-4">
                    ...
                    <v-select
                        v-model="data.default_grade"
                        :items="defaultGradeItems"
                        item-title="title"
                        item-value="value"
                        label="Standardnote (optional)"
                        clearable
                        class="mt-2" />
...
export default {
    ...
    computed: {
        ...
        defaultGradeItems() {
            return this.sortedGrades(this.data?.grades || []).map((grade) => ({
