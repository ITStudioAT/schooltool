...
class RegisterDateService
{
    ...
    public function createDates($school_id, $schoolyear_id, $register_id, array $data): bool
    {
        ...
        foreach (range(1, 5) as $i) {
            if (! empty($data["supervisor_$i"])) {
                $supervisors[] = $data["supervisor_$i"];
            }
        }
        ...
        $startDate = Carbon::parse($date_from, $tz)->startOfDay();
        $endDate = Carbon::parse($date_until, $tz)->endOfDay();
        ...
        foreach (CarbonPeriod::create($startDate, '1 day', $endDate) as $date) {
            if (! in_array(strtolower($date->englishDayOfWeek), $wantedDays, true)) {
                continue;
            }
