...
class MaterialStorageSyncStatusStore
{
     ...
     */
    public function createOperation(int $userId, int $schoolId): array
    {
     ...
     */
    public function markCompleted(int $userId, string $operationId, array $result, string $message = 'Der Download der Materialdateien ist abgeschlossen.'): void
    {
        $payload = $this->getOperation($userId, $operationId);
        if (! is_array($payload)) {
            return;
    ...
    {
        return 'materials:storage-sync:'.$userId.':'.$operationId;
    }
}
