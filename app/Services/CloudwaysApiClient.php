...
class CloudwaysApiClient
{
    ...
    public function startGitPull(): void
    {
        ...
        try {
            $response = $this->request()->asForm()->post('/git/pull', $payload);
            $this->throwForFailure($response);
        } catch (ConnectionException $exception) {
            throw new RuntimeException('Could not connect to the Cloudways deployment API.', previous: $exception);
        } catch (RequestException $exception) {
            throw $this->sanitizedRequestException($exception);
        }
    }
    ...
    private function sanitizedRequestException(RequestException $exception): RuntimeException
    {
        return new RuntimeException(
            "Cloudways API request failed with HTTP status {$exception->response->status()}.",
            previous: $exception,
        );
    }
