...
#[Signature('preview:snapshot {action : status, key:generate, export, delete, receive, import, restore, checkpoint, assert-current, assert-plan, activate or complete-main} {--feature=} {--recipient=} {--artifact=} {--sha256=} {--source=} {--state-token=} {--path-only : Print only the verified received path} {--replace : Explicitly confirmed replacement of preview data}')]
    ...
    public function handle(FeaturePreviewSnapshotService $snapshots): int
    {
        try {
            ...
            $result = match ($this->argument('action')) {
                ...
                'complete-main' => $this->completeOperation(fn () => $snapshots->completeMain((string) $this->option('source'))),
