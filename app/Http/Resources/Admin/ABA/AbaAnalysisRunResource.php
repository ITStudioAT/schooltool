...
class AbaAnalysisRunResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        $textLength = $analysisStats['text_length'] ?? $this->text_length ?? null;
        $textLengthWithoutSpaces = $analysisStats['text_length_without_spaces'] ?? $this->text_length_without_spaces ?? null;

        if (! is_array($analysisStats)) {
            $analysisStats = [];
        }
        $analysisStats['text_length'] = is_numeric($textLength) ? (int) $textLength : null;
        $analysisStats['text_length_without_spaces'] = is_numeric($textLengthWithoutSpaces) ? (int) $textLengthWithoutSpaces : null;

        if ($displayValues === []) {
            $displayValues = [
                'total_record_count' => (int) ($analysisStats['persisted_record_count'] ?? $this->extracted_sections_count ?? 0),
                'text_length' => is_numeric($textLength) ? (int) $textLength : null,
                'text_length_without_spaces' => is_numeric($textLengthWithoutSpaces) ? (int) $textLengthWithoutSpaces : null,
                ...
                'auto_approve_confidence_threshold' => is_numeric($analysisStats['auto_approve_confidence_threshold'] ?? null)
                    ? (float) $analysisStats['auto_approve_confidence_threshold']
                    : (float) config('aba_analysis.auto_approve_confidence', 0.82),
                'analysis_start_at' => $this->started_at?->toISOString(),
                'analysis_end_at' => $this->completed_at?->toISOString() ?? $this->failed_at?->toISOString() ?? $this->aborted_at?->toISOString(),
                'title_page_details' => [
                    'title' => $analysisStats['title_page_title'] ?? null,
                    'submitter' => $analysisStats['title_page_submitter'] ?? null,
                    'advisor' => $analysisStats['title_page_advisor'] ?? null,
                    'class' => $analysisStats['title_page_class'] ?? null,
                    'year' => $analysisStats['title_page_year'] ?? null,
                ],
                'title_page_title' => $analysisStats['title_page_title'] ?? null,
                'title_page_submitter' => $analysisStats['title_page_submitter'] ?? null,
                'title_page_advisor' => $analysisStats['title_page_advisor'] ?? null,
                'title_page_class' => $analysisStats['title_page_class'] ?? null,
                'title_page_year' => $analysisStats['title_page_year'] ?? null,
            ];
        }
        ...
        return [
            ...
            'text_length' => is_numeric($this->text_length) ? (int) $this->text_length : null,
            'text_length_without_spaces' => is_numeric($this->text_length_without_spaces) ? (int) $this->text_length_without_spaces : null,
            'persisted_record_count' => (int) ($analysisStats['persisted_record_count'] ?? $this->extracted_sections_count ?? 0),
