...
class UpdateRestaurantMenuPlanRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'entries.*.food_ids' => ['nullable', 'array'],
            'entries.*.food_ids.*' => ['integer', 'distinct', 'exists:restaurant_foods,id'],
            'entries.*.foods' => ['nullable', 'array'],
            'entries.*.foods.*.id' => ['nullable', 'integer', 'exists:restaurant_foods,id'],
            'entries.*.foods.*.title' => ['required_with:entries.*.foods', 'string', 'max:255'],
            'entries.*.foods.*.description' => ['nullable', 'string', 'max:4000'],
            'entries.*.foods.*.allergens' => ['nullable', 'array'],
            'entries.*.foods.*.allergens.*' => ['string', 'max:80'],
            'entries.*.foods.*.price' => ['nullable', 'numeric', 'min:0', 'max:9999.99'],
            'entries.*.foods.*.food_image_url' => ['nullable', 'string', 'max:2048'],
            'entries.*.foods.*.food_image_path' => ['nullable', 'string', 'max:255'],
            'entries.*.foods.*.category' => ['nullable', 'array'],
            'entries.*.foods.*.category.id' => ['nullable', 'integer', 'exists:restaurant_categories,id'],
            'entries.*.foods.*.category.title' => ['nullable', 'string', 'max:120'],
            'entries.*.foods.*.ingredient_icons' => ['nullable', 'array'],
            'entries.*.foods.*.ingredient_icons.*.id' => ['nullable', 'integer', 'exists:restaurant_ingredient_icons,id'],
            'entries.*.foods.*.ingredient_icons.*.title' => ['nullable', 'string', 'max:120'],
            'entries.*.foods.*.ingredient_icons.*.image_url' => ['nullable', 'string', 'max:2048'],
            'entries.*.foods.*.ingredient_icons.*.image_path' => ['nullable', 'string', 'max:255'],
