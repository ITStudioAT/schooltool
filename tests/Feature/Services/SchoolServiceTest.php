...
describe('deleteSchools', function () {
    it('prevents deletion of school with ID 1', function () {
        $this->service->deleteSchools([1]);
    })->throws(Exception::class, 'Die Big-Boss-Schule kann nicht gelöscht werden.');

    it('prevents deletion of school with registers', function () {
        // Create school with ID != 1
        $school = School::factory()->create(['id' => 999]);
        Register::factory()->create(['school_id' => $school->id]);

        $this->service->deleteSchools([$school->id]);
    })->throws(Exception::class, 'Mindestens eine Schule ist einem Registrierungstool zugeordnet.');

    it('prevents deletion of school with multiple users', function () {
        // Create school with ID != 1
        $school = School::factory()->create(['id' => 998]);
        ...
        $this->service->deleteSchools([$school->id]);
    })->throws(Exception::class, 'Bei mindestens einer Schule sind noch Benutzer zugeordnet.');

    it('successfully deletes school with only super admin', function () {
        // Create school with ID != 1
        $school = School::factory()->create(['id' => 997]);
        $schoolyear = Schoolyear::factory()->create(['school_id' => $school->id]);
    ...
    it('detaches all licences when deleting school', function () {
        // Create school with ID != 1
        $school = School::factory()->create(['id' => 996]);
        $schoolyear = Schoolyear::factory()->create(['school_id' => $school->id]);
        $user = User::factory()->create([
            'school_id' => $school->id,
            'schoolyear_id' => $schoolyear->id,
