...
it('ignores a persisted v3 timetable generated by an unknown or older algorithm', function (array $summary) {
    ...
    'older version' => [['algorithm_version' => 7, 'timetable_count' => 18]],
]);
...
it('creates, updates, and reuses all possible v3 timetables for a planning context', function () {
    ...
    expect($created)
        ->status->toBe('created')
        ->reused->toBeFalse()
        ->summary->algorithm_version->toBe(8)
...
it('generates only selected F2 when the compact subject plan also contains Latin and Spanish', function () {
    [$user, $schoolyear] = v3TimetableServiceUser();
    $studentCode = 'student-french-2';

    collect([
        ['code' => 'L2', 'subject' => 'L', 'name' => 'Latein 2'],
        ['code' => 'F2', 'subject' => 'F', 'name' => 'Französisch 2'],
        ['code' => 'S2', 'subject' => 'S', 'name' => 'Spanisch 2'],
    ])->each(function (array $subject, int $index) use ($user, $schoolyear): void {
        StudentTimetableSubjectRow::query()->create([
            'school_id' => $user->school_id,
            'schoolyear_id' => $schoolyear->id,
            'study_program' => StudentTimetableStudyProgram::Kompaktstudium,
    ...
    $information['student_code'] = $studentCode;
    $information['study_program'] = StudentTimetableStudyProgram::Kompaktstudium->value;
    $information['semester'] = 3;
    $information['module_selection_groups'][0] = [
        'key' => 'previous',
        'label' => 'Frühere Module',
        'modules' => [[
            'selection_key' => 'previous:F2',
            'code' => 'F2',
            'name' => 'Französisch 2',
            'hours' => 4,
            'courses' => [
                ['key' => 'f2-classroom', 'keys' => ['f2-classroom'], 'title' => 'F2 - 3C - SCHO'],
                ['key' => 'f2-distance', 'keys' => ['f2-distance'], 'title' => 'F2 - 2QS+3K - SCHO'],
            ],
        ]],
    ];
    $courseGroups = [
        v3TimetableServiceCourseGroup('f2-classroom', 'F2 - 3C - SCHO', 'F2', 1, 1),
        v3TimetableServiceCourseGroup('f2-distance', 'F2 - 2QS+3K - SCHO', 'F2', 5, 2),
    ];
    $service = v3TimetableService($user, $information, $courseGroups, $studentCode);

    $result = $service->createOrUpdateForUser($user, ['previous:F2'], [
        'planning_mode' => 'with_student',
        'student_code' => $studentCode,
        'selected_course_keys' => ['f2-classroom', 'f2-distance'],
    ]);
    ...
    expect($result)
        ...
        ->summary->timetable_variation_count->toBe(2)
        ->summary->timetable_count->toBe(2)
        ->modules->toHaveCount(1)
        ->and(collect($result['modules'])->pluck('code')->all())->toBe(['F2'])
