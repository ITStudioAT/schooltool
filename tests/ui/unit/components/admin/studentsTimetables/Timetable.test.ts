...
describe('Students timetable timetable page', () => {
    it('shows the timetable overview on the timetable overview subpage', () => {
        ...
        expect(componentSource).toContain("import EvaluationSettings from '../evaluationSettings/EvaluationSettings.vue'")
        ...
        expect(componentSource).toContain('<EvaluationSettings v-else-if="subAction === \'settings\'" />')
        expect(componentSource).toContain("{ key: 'overview', label: 'Übersicht' }")
        expect(componentSource).toContain("{ key: 'robot', label: 'Roboter' }")
        expect(componentSource).toContain("{ key: 'settings', label: 'Einstellungen' }")
        ...
        expect(componentSource.indexOf("{ key: 'robot', label: 'Roboter' }"))
            .toBeLessThan(componentSource.indexOf("{ key: 'settings', label: 'Einstellungen' }"))
        expect(componentSource).toContain("const allowed = ['overview', 'robot', 'settings', 'imports']")
        expect(componentSource).toContain('v-if="showTimetableSubnav"')
        expect(componentSource).toContain("return this.subAction !== 'imports'")
    ...
    it('shows import buttons that open import subpages', () => {
        ...
        expect(componentSource).toContain("const shouldLoadFullSubjectImports = this.activeImportPage === 'faecher'")
        ...
        expect(componentSource).toContain('summary: shouldLoadFullSubjectImports ? 0 : 1')
    ...
    it('formats import button metadata', () => {
        ...
        expect((Timetable as any).computed.subjectImportPrompt()).toContain('F1/L1/S1*')
        expect((Timetable as any).computed.subjectImportPrompt()).toContain('BE1 und ME1')
        expect((Timetable as any).computed.subjectImportPrompt()).toContain('Wahlalternativen werden als eigene Kursobjekte importiert')
