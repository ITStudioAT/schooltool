...
describe('Students timetable subjects overview', () => {
    ...
    it('adds the subjects overview menu item to the module shell', () => {
        ...
        expect(componentSource).toContain("key: 'import'")
        ...
        expect(componentSource).toContain("meta: 'Import'")
        ...
        expect(componentSource).toContain('/admin/students-timetables/subjects-overview/overview')
    ...
    it('splits the subjects area into overview, subject plan, import, subjects, and mapping pages', () => {
        const componentSource = readFileSync(
        ...
        expect(componentSource).toContain("v-if=\"['overview', 'subject-plan'].includes(subject_action)\"")
    ...
    it('uses FilePond upload for json files', () => {
        ...
        expect(componentSource).toContain('Analyse')
        expect(componentSource).toContain('Keine Fächer oder Semester in der JSON-Datei erkannt.')
        expect(componentSource).toContain('importSubjectCountLabel(importItem.analysis)')
        expect(componentSource).toContain('analysis?.subjects_total')
        expect(componentSource).toContain('Fächer / ${courseRowsTotal} Kurse')
        expect(componentSource).toContain('semester.subjects')
        expect(componentSource).toContain('semester.branch_variants?.length')
        ...
        expect(componentSource).toContain('branch.common_subjects')
        expect(componentSource).toContain('branch.different_subjects')
        expect(componentSource).toContain('branch-different-subjects')
        expect(componentSource).toContain('branchVariantClass(branch.key)')
        expect(componentSource).toContain('branch-variant-block--common')
        expect(componentSource).toContain('branch-variant-block--wirtschaft')
