...
describe('Grading settings required entries chip', () => {
    ...
    it('places the edit pencil in the Benotung header actions', () => {
        ...
        expect(source).toContain('v-btn v-if="is_editing && !isLocalSemesterCountEditing && !isLocalSemesterWeightEditing && !isLocalSemesterInclusionEditing" icon="mdi-check" size="x-small" color="success" variant="flat" :loading="isSavingGrading" :disabled="!isCurrentEditValid || isSavingGrading" @click="save"')
        expect(source).toContain('v-btn v-if="is_editing && !isLocalSemesterCountEditing && !isLocalSemesterWeightEditing && !isLocalSemesterInclusionEditing" icon="mdi-close" size="x-small" color="warning" variant="flat" :disabled="isSavingGrading" @click="exitEditMode"')
        ...
        expect(source).toContain(':disabled="!isCurrentEditValid || isSavingGrading"')
        expect(source).toContain(':loading="isSavingGrading"')
